...
const saveBookmark = async (postId, rawName) => {
  ...
  return (
    ...
    <div className="two secondd flex-1 bg-black snap-y overflow-y-scroll scrollbar-hide overflow-x-hidden">
      {showcomments &&   <CommentSection
       ...
       showGifUpload={showGifUpload}
       setShowGifUpload={setShowGifUpload}
      ...
      // postId={postId}
       ...
       avatar={avatar}
       iscross = {setshowComments}
     />}
      <div className="headers textu flex h-12 sticky top-0 bg-black z-10 bg-opacity-70">
        <div
      ...
      <div className="yourpost h-[60vh]">
        ...
        <div className="content flex flex-col puttr h-[500px] overflow-y-auto overflow-x-hidden px-4">
          {posts?.length === 0 ? (
            <div></div>
          ) : (
            posts.map((post, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className='relative left-[400px] bottom-4 text-gray-300 text-sm '><p> Posted On :{(post?.createdAt).slice(0, 10)}</p></div>
                  ...
                  <div className='flex justify-between relative right-2 post-card'>
                    ...
                    <div className='flex justify-between items-center gap-4 space-x-5 my-5  relative left-5 py-10 '>
                      ...
                      <div className='comment' onClick={()=>{setshowComments(true)}}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={24} height={24} color={"#ffffff"} fill={"none"}>
  ...
  <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
    <div className="bg-zinc-900 p-6 rounded-lg w-[300px] space-y-3 text-white">
      <h2 className="text-lg font-semibold">Save to Collection</h2>

      {collections.map((col, idx) => (
        <div
          key={idx}
          className="p-2 border border-gray-600 rounded hover:bg-gray-700 cursor-pointer"
      ...
      />
      <button
        onClick={() => saveBookmark(selectedPostId, newCollectionName)}
    ...
    <div className="bg-zinc-900 p-6 rounded-xl w-[90%] max-w-sm text-white space-y-5 shadow-2xl border border-gray-700">
      ...
      <div className="flex flex-col space-y-2 max-h-40 overflow-y-auto">
        ...
        {/* {collections.map((col, idx) => (
          <div
            key={idx}
            className={`px-4 py-2 border rounded-lg cursor-pointer hover:bg-gray-700 transition ${
              col === newCollectionName ? 'bg-blue-600 border-blue-500' : 'border-gray-600'
            }`}
            onClick={() => saveBookmark(selectedPostId, col)}
          >
            {col}
  ...
  .map((col, idx) => (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {/* <button
          disabled={!newCollectionName.trim() || collections.includes(newCollectionName.trim())}
          onClick={() => saveBookmark(selectedPostId, newCollectionName.trim())}
          className={`w-full py-2 rounded text-sm font-semibold transition ${
            !newCollectionName.trim() || collections.includes(newCollectionName.trim())
              ? 'bg-gray-700 text-gray-400 cursor-not-allowed'
              : 'bg-blue-500 hover:bg-blue-600'
          }`}
        >
          {collections.includes(newCollectionName.trim()) ? 'Collection already exists' : 'Create & Save'}
        </button> */}
        <button
  disabled={
    ...
    : 'Create & Save'}
      ...
      >
        Cancel
          ...
          <div ref={loaderRef} className="h-10 mt-4 flex justify-center items-center text-gray-400">
